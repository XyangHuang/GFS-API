...
{
    @Query("FROM QuestionChoice c WHERE c.questionId=:questionId AND c.markForDelete=0")
    public List<QuestionChoice> getChoicesByQuestionId(@Param("questionId") long questionId);
    
    @Query("SELECT c.id FROM QuestionChoice c WHERE c.questionId=?1 AND c.markForDelete=0")
    public List<Long> getChoiceIdsByQuestionId(long questionId);
    
    @Modifying
    @Transactional
    @Query("UPDATE QuestionChoice c SET c.markForDelete=1 WHERE c.id=?1 AND c.markForDelete=0")
    public int deleteChoiceByIdLogically(long choiceId);

    @Modifying
    @Transactional
    @Query("UPDATE QuestionChoice c SET c.markForDelete=1 WHERE c.questionId=?1 AND c.markForDelete=0")
    public int deleteChoicesByQuestionIdLogically(long questionId);

    @Modifying
    @Transactional
    @Query("DELETE FROM QuestionChoice c WHERE c.questionId=?1")
